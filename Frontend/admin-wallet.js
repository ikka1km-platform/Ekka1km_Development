...
AdminModules.register("wallet", async function(container) {
  ...
  function renderEconomyRules() {
    ...
    html += passes.length ? '<div style="font-size:12px;line-height:1.8;">' + passes.map(function (p) { return '<div><strong>' + esc(p.passName) + '</strong>: ₹' + esc(p.priceINR) + ' · ' + esc(p.coinAllocation ? p.coinAllocation.label : (p.includedCoins + ' Coins')) + '</div>'; }).join("") + '</div>' : '<div style="font-size:12px;color:var(--text-muted);">No active promotion passes configured.</div>';
